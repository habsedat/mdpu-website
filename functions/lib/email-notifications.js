...
const createApprovalEmail = (applicantName, email) => ({
...
const createRejectionEmail = (applicantName, email) => ({
...
exports.sendApprovalEmail = (0, https_1.onCall)(async (request) => {
    try {
        const { email, fullName } = request.data;
        const { auth } = request;
        // Check if user is admin
        if (!auth || !auth.token.role || (auth.token.role !== 'admin' && auth.token.role !== 'superadmin')) {
            throw new https_1.HttpsError('permission-denied', 'Only admins can send approval emails');
        }
        if (!email || !fullName) {
        ...
        throw new https_1.HttpsError('internal', `Failed to send approval email: ${error.message}`);
    }
});
// Callable function to send rejection notification to applicant
exports.sendRejectionEmail = (0, https_1.onCall)(async (request) => {
    try {
        const { email, fullName } = request.data;
        const { auth } = request;
        // Check if user is admin
        if (!auth || !auth.token.role || (auth.token.role !== 'admin' && auth.token.role !== 'superadmin')) {
            throw new https_1.HttpsError('permission-denied', 'Only admins can send rejection emails');
        }
        if (!email || !fullName) {
            throw new https_1.HttpsError('invalid-argument', 'Email and fullName are required');
        }
        ...
        throw new https_1.HttpsError('internal', `Failed to send rejection email: ${error.message}`);
    }
});
